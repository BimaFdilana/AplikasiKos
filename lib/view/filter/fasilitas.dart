...
class FasilitasCategory extends StatefulWidget {
  const FasilitasCategory({super.key});

  @override
  State<FasilitasCategory> createState() => _FasilitasCategoryState();
}

class _FasilitasCategoryState extends State<FasilitasCategory> {
  var loading = false;
  final list = <KosModel>[];
  ...
  getPref() async {
    ...
    setState(() {
      pilihFkos = preferences.getStringList("pilihFkos") ?? [];
      pilihFkeaman = preferences.getStringList("pilihFkeaman") ?? [];
      pilihFumum = preferences.getStringList("pilihFumum") ?? [];
    });
  }
  ...
  Future<void> lihatKos() async {
    ...
    final response = await http.get(Uri.parse(
        'https://bengkaliskost.com/api/filter.php?f_kos=$pilihFkosString&fkeaman=$pilihFkeamanString&fumum=$pilihFumumString'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Categori Campur'),
